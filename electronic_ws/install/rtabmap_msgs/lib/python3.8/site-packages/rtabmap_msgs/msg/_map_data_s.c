...
bool rtabmap_msgs__msg__node_data__convert_from_py(PyObject * _pymsg, void * _ros_message);
...
bool rtabmap_msgs__msg__map_data__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // nodes
    ...
    if (!rtabmap_msgs__msg__NodeData__Sequence__init(&(ros_message->nodes), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create rtabmap_msgs__msg__NodeData__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    rtabmap_msgs__msg__NodeData * dest = ros_message->nodes.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!rtabmap_msgs__msg__node_data__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * rtabmap_msgs__msg__map_data__convert_to_py(void * raw_ros_message)
{
  ...
  {  // nodes
    ...
    rtabmap_msgs__msg__NodeData * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->nodes.data[i]);
      PyObject * pyitem = rtabmap_msgs__msg__node_data__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
