  ...
  lslidar_msgs__msg__LslidarInformation__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lslidar_msgs__msg__LslidarInformation * data =
      (lslidar_msgs__msg__LslidarInformation *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lslidar_msgs__msg__LslidarInformation__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          lslidar_msgs__msg__LslidarInformation__fini(&data[i]);
        }
        free(data);
        return false;
      }
